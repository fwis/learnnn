...
CLIP_STRIDES = 40
#PATCH_NUM = 10
# BIC_filename = '../ForkNet-master/data/training_set/BIC.h5'
Y_filename = '../ForkNet-master/data/training_set/Y.h5'
labels_filename = '../ForkNet-master/data/training_set/Labels.h5'
data_path = '../ForkNet-master/data/training_set/'

# 确保目标文件夹存在
# os.makedirs(os.path.dirname(BIC_filename), exist_ok=True)
...
Para[:,:,:,0] = 1/2*(X[:,:,:,0] + X[:,:,:,1] + X[:,:,:,2] + X[:,:,:,3]) # S0
Para[:,:,:,1] = dolp(X[:,:,:,0], X[:,:,:,1], X[:,:,:,2], X[:,:,:,3]) # DoLP
Para[:,:,:,2] = aop(X[:,:,:,0], X[:,:,:,1], X[:,:,:,2], X[:,:,:,3]) + math.pi/4. # AoP
