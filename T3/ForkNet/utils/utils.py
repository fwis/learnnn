...
def plot_feature_map(conv_img, plot_dir, maps_all=True):
    if maps_all:
        num_maps = conv_img.shape[1]
        maps = range(conv_img.shape[1])
    else:
        maps = np.linspace(0, conv_img.shape[1], 8, endpoint=False, dtype=int)
        num_maps = len(maps)

    for map_idx in maps:
        img = normalize(conv_img[0, map_idx].detach().cpu().numpy(), 1, 0)
        imgio.imwrite(os.path.join(plot_dir, '{}.jpg'.format(map_idx)), img)


def fig2array(fig):
    """
    把matlplotlib绘制的图像转为RGBA格式的numpy数组
    """
    # draw the renderer
    fig.canvas.draw()
