...
def train(model, train_loader, val_loader, device, num_epochs, learning_rate=0.001, weight_decay=1e-4, checkpoint_path='T3/SR/ckpt/ResNet_sr_3.pth', savebest=True):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (data, labels) in enumerate(train_loader):
            ...
            with autocast(): 
                ...
                perceptual_loss = (perceptual_criterion(output[:][0],labels[:][0]) + 
                                   perceptual_criterion(output[:][1],labels[:][1]) + 
                                   perceptual_criterion(output[:][2],labels[:][2]) + 
                                   perceptual_criterion(output[:][3],labels[:][3]))/4
                # print('perceptual loss', perceptual_loss)
                loss = content_loss + 1e-3 * perceptual_loss
                # print('loss',loss)
